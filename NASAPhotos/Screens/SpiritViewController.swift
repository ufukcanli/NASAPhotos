...
class SpiritViewController: UIViewController {
    ...
    private func performRequest(byPage page: Int) {
        isLoading = true
        NASADataManager.shared.getSpiritPhotos(page: currentPage) { [weak self] result in
    ...
    private func performRequest(byCamera camera: String) {
        isLoading = true
        NASADataManager.shared.getSpiritPhotos(byCamera: camera) { [weak self] result in
            guard let self = self else { return }
            switch result {
            case .success(let results):
                self.photos.append(contentsOf: results.photos)
    ...
    private func configureViewController() {
        ...
        navigationItem.title = "Spirit - \(currentFilter)"
...
extension SpiritViewController: NASACameraViewControllerDelegate {
    func didFinishPickingCamera(camera: String) {
        guard !isLoading else { return }
        photos.removeAll()
        currentFilter = camera.uppercased()
        navigationItem.title = "Spirit - \(camera.uppercased())"
        if camera != "all" {
